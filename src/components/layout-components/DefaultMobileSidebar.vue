      ...
      <aside v-if="isMobileSidebarOpen" class="default-mobile-sidebar lg:hidden">
        ...
        <nav class="h-[calc(100vh-88px)] flex flex-col justify-between">
          <ul>
            <SidebarLink v-for="sidebarLink in sidebarLinks" :key="sidebarLink.to" :to="sidebarLink.to"
              ...
              <template #icon>
                ...
                <healthicons-i-exam-multiple-choice-negative v-else-if="sidebarLink.text === 'Exams'" class="text-[1.5rem]" />
          ...
          <ul class="bottom-links">
            <SidebarLink to="/logout" text="Logout" :is-sidebar-open="isMobileSidebarOpen" class="text-[1.2rem]"
              :is-button="true">
              <template #icon>
